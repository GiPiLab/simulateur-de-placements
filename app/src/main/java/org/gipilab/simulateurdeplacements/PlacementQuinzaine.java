...
public class PlacementQuinzaine extends Placement {
    ...
    ArrayList<Echeance> tableauPlacement() {
        ...
        for (i = 1; i <= this.duree; i++) {
            ...
            //Si on a dépose avant le 16 on doit supprimer une quinzaine d'intérêts
            if ((i == 1 || i == duree) && cal.getDayOfMonth() < 16) {
                mensualite.setInteretsObtenus(capitalPlace.multiply(tauxQuinzaine, MathContext.DECIMAL128));
            } else {
        ...
        if (daysLeft > 0 && dateFin.getDayOfMonth() < 16) {
